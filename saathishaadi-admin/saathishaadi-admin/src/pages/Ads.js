...
export default function Ads() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editAd) {
        await api.updateAd(editAd._id, form);
        toast.success('Ad update ho gaya');
      } else {
        await api.createAd(form);
        toast.success('Naya ad create ho gaya');
      }
      setShowModal(false);
      loadAds();
    } catch (err) {
  ...
  const handleToggle = async (ad) => {
    try {
      await api.updateAd(ad._id, { ...ad, isActive: !ad.isActive });
      toast.success(ad.isActive ? 'Ad deactivate kiya' : 'Ad activate kiya');
      loadAds();
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast.success('Ad delete ho gaya');
      ...
      <div className="content-body">
        ...
        <div className="ad-stats-row">
          <div className="ad-stat-card">
            <div className="ad-stat-num">{ads.length}</div>
            <div className="ad-stat-label">Total Ads</div>
          </div>
          <div className="ad-stat-card" style={{ '--c': '#27ae60' }}>
            <div className="ad-stat-num" style={{ color: '#27ae60' }}>{ads.filter(a => a.isActive).length}</div>
            <div className="ad-stat-label">Active</div>
          </div>
          <div className="ad-stat-card" style={{ '--c': '#e74c3c' }}>
            <div className="ad-stat-num" style={{ color: '#e74c3c' }}>{ads.filter(a => !a.isActive).length}</div>
            <div className="ad-stat-label">Inactive</div>
          </div>
          <div className="ad-stat-card" style={{ '--c': '#d4a017' }}>
            <div className="ad-stat-num" style={{ color: '#d4a017' }}>{ads.reduce((s, a) => s + (a.clicks || 0), 0)}</div>
            <div className="ad-stat-label">Total Clicks</div>
          </div>
        </div>
            ...
            {ads.map(ad => (
              <div key={ad._id} className={`ad-card ${!ad.isActive ? 'ad-inactive' : ''}`}>
                <div className="ad-preview" style={{ background: ad.bgColor }}>
                  {ad.imageUrl ? (
                    <img src={ad.imageUrl} alt={ad.title} className="ad-preview-img" onError={e => e.target.style.display = 'none'} />
                  ) : (
                ...
                <div className="ad-card-body">
                  <div className="ad-card-meta">
                    <span className={`badge ${ad.isActive ? 'badge-green' : 'badge-red'}`}>
                  ...
                  <div className="ad-card-actions">
                    ...
                    <button className={`btn-sm ${ad.isActive ? 'btn-danger' : 'btn-success'}`} onClick={() => handleToggle(ad)}>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="form-group">
                ...
                <textarea className="input" placeholder="Ad description (optional)" value={form.description} onChange={f('description')} rows={3} />
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label>Image URL</label>
                  <input className="input" placeholder="https://..." value={form.imageUrl} onChange={f('imageUrl')} />
                </div>
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <div className="color-picker">
                  {BG_COLORS.map(c => (
                    <div
                      key={c}
                      className={`color-swatch ${form.bgColor === c ? 'selected' : ''}`}
                      style={{ background: c }}
              ...
              <div className="form-group">
                <label>Preview</label>
                <div className="ad-preview-mini" style={{ background: form.bgColor }}>
                  {form.imageUrl && <img src={form.imageUrl} alt="" className="ad-preview-img" onError={e => e.target.style.display = 'none'} />}
                  <div className="ad-preview-overlay">
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                <button className="btn-primary" type="submit" disabled={saving}>{saving ? 'Save ho raha...' : '💾 Save'}</button>
        ...
        <div className="modal-overlay" onClick={() => setDeleteConfirm(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <p style={{ marginBottom: 20, color: 'var(--text-muted)' }}>
              "<strong>{deleteConfirm.title}</strong>" ko permanently delete karna chahte hain?
            </p>
